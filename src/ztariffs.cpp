...
{
	if (l_Modes.size() == 0)
		l_Modes << "станок" << "модель шины содержит ..." << "модель шины начинается с ..." << "модель шины заканчивается ..." << "Rate содержит ..." << "станок начинается с ..." << "станок содержит ..." << "станок заканчивается ...";
	return l_Modes;
}
...
void ZTariffs::init()
{
	...
	hideColumns << 0;
	headers << tr("id") << tr("Название") << tr("Режим") << tr("Доплата") << tr("Вид") << tr("Комментарий");
